...
public class KnightMovesCalculator implements PieceMovesCalculator{
    ...
    public Collection<ChessMove> calculateMoves (ChessBoard board, ChessPosition position, ChessPiece piece) {
        ...
        applyKnightMoves(board, position, moves);
        return moves;
    }
    ...
    private void applyKnightMoves(ChessBoard board, ChessPosition position, Collection<ChessMove> moves) {
        for(int[] move: KNIGHT_MOVES) {
            ChessPosition endPosition = getPosition(position, move[0], move[1]);
            validateAndAddPosition(board, position, endPosition, moves);
        }
    }
}
